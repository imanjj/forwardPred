local forwardPred, parent = torch.class('nn.forwardPred', 'nn.Module')

function forwardPred:__init(inputSize, outputSize, bias)
   parent.__init(self)
...
function forwardPred:noBias()
   self.bias = nil
   self.gradBias = nil
   return self
end

function forwardPred:reset(stdv)
   if stdv then
...
function forwardPred:updateOutput(input)
...
function forwardPred:updateGradInput(input, gradOutput)
   ...
   if self.gradInput then
	 ...
	 --print(torch.cmul(self.output, torch.ge(self.output, 0):double()))
         print('weight pow 2:')
         print(torch.pow(self.weight,2))
         -- self.gradInput:addmm(0, 1, gradOutput, self.weight)
	 self.gradInput:addmm(0, -1, self.output, torch.pow(self.weight,2))
         --self.gradInput:addmm(0, -1, torch.tanh(self.output), torch.pow(self.weight,2))
         print('self.gradInput after first computation:')
         print(self.gradInput)
	 print('torch.norm(self.weight,2,1):pow(2):expandAs(input)')
 	 print(torch.norm(self.weight,2,1):pow(2):expandAs(input))
	 print('input')
	 print(input)
	 self.gradInput:addcmul(1, torch.norm(self.weight,2,1):pow(2):expandAs(input), input)
...
function forwardPred:accGradParameters(input, gradOutput, scale)
...
forwardPred.sharedAccUpdateGradParameters = forwardPred.accUpdateGradParameters

function forwardPred:clearState()
   if self.addBuffer then self.addBuffer:set() end
   return parent.clearState(self)
end

function forwardPred:__tostring__()
